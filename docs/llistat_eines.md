Aquest document presenta el llistat d’eines digitals (integrades o pendents d’integrar) de la plataforma FREC.  Això inclou tant aplicacions que compten amb una versió en català com aplicacions que, tot i no complir aquest requisit, tenen un ús ampli dins el teixit catalanoparlant i per tant sembla clar que incorporar-les a FREC serà beneficiós pels usuaris en aquesta llengua. 

Podeu consultar si han estat integrats a la plataforma FREC en aquesta taula. ***

# Catàleg d’eines (ordenat alfabèticament)

## A Better Routeplanner (ABRP)
| Camp | Valor |
|------|------|
| Nom | A better Routeplanner (ABRP) |
| Enllaç | https://abetterrouteplanner.com/ |
| Desenvolupador | Rivian Automotive, Inc. |
| Funció | Planificador de rutes |
| Àrea | Mapes i viatges (MA-VI) |
| Descripció | Planificador de viatges per vehicles elèctrics amb rutes i punts de càrrega. |

## Apertium
| Camp | Valor |
|------|------|
| Nom | Apertium |
| Enllaç | https://www.apertium.org/index.cat.html#?dir=cat-epo&q= |
| Desenvolupador | Apertium |
| Funció | Traductor |
| Àrea | Llengua (LLGA) |
| Descripció | Traductor obert especialment eficient entre llengües romàniques. |

## ChatGPT - Open AI
| Camp | Valor |
|------|------|
| Nom | ChatGPT |
| Enllaç | https://chatgpt.com/ |
| Desenvolupador | Open AI |
| Funció | Plataforma conversacional |
| Àrea | Motor IA (IA) |
| Descripció | IA conversacional per generar contingut, resoldre dubtes i assistència lingüística. |

## Catàleg de dades obertes - Portal OpenData AMB
| Camp | Valor |
|------|------|
| Nom | Catàleg de dades obertes - Portal OpenData AMB |
| Enllaç | https://www.amb.cat/s/web/area-metropolitana/dades-obertes.html |
| Desenvolupador | Àrea Metropolitana de Barcelona |
| Funció | Proveïment de dades |
| Àrea | General (GRAL) |
| Descripció | Portal de dades obertes amb conjunts de dades reutilitzables via API. |

## Corrector ortogràfic i gramatical - SoftCatalà
| Camp | Valor |
|------|------|
| Nom | Corrector ortogràfic i gramatical |
| Enllaç | https://www.softcatala.org/corrector/ |
| Desenvolupador | SoftCatalà |
| Funció | Corrector |
| Àrea | Llengua (LLGA) |
| Descripció | Eina per detectar i corregir errors ortogràfics i gramaticals en català. |

## DeepSeek - DeepSeek AI
| Camp | Valor |
|------|------|
| Nom | DeepSeek |
| Enllaç | https://www.deepseek.com/ |
| Desenvolupador | DeepSeek AI |
| Funció | Plataforma conversacional |
| Àrea | Motor IA |
| Descripció | Motor de cerca amb IA per analitzar i resumir informació. |

## Diccionari de la Llengua Catalana - IEC
| Camp | Valor |
|------|------|
| Nom | Diccionari de la Llengua Catalana - DIEC2 |
| Enllaç | https://deiec.iec.cat/ |
| Desenvolupador | Institut d’Estudis Catalans |
| Funció | Diccionari |
| Àrea | Llengua (LLGA) |
| Descripció | Diccionari normatiu amb definicions i informació lingüística. |

## Diccionari de sinònims - SoftCatalà
| Camp | Valor |
|------|------|
| Nom | Diccionari de sinònims |
| Enllaç | https://www.softcatala.org/diccionari-de-sinonims/ |
| Desenvolupador | SoftCatalà |
| Funció | Diccionari |
| Àrea | Llengua (LLGA) |
| Descripció | Cercador de sinònims en català. |

## Furkot
| Camp | Valor |
|------|------|
| Nom | Furkot |
| Enllaç | https://trips.furkot.com/ |
| Desenvolupador | LewyHals |
| Funció | Planificador de rutes |
| Àrea | Mapes i viatges (MA-VI) |
| Descripció | Planificador d’itineraris amb allotjaments i activitats. |

## Gmail - Google Inc.
| Camp | Valor |
|------|------|
| Nom | Gmail |
| Enllaç | https://mail.google.com/mail/u/0/ |
| Desenvolupador | Google Inc. |
| Funció | Correu electrònic |
| Àrea | Comunicació (COM) |
| Descripció | Servei de correu electrònic amb filtres i integració Google. |

## Google Calendar - Google Inc.
| Camp | Valor |
|------|------|
| Nom | Google Calendar |
| Enllaç | https://calendar.google.com/calendar |
| Desenvolupador | Google Inc. |
| Funció | Gestor d’agenda |
| Àrea | Productivitat (PROD) |
| Descripció | Gestió d’esdeveniments i sincronització amb serveis Google. |

## Google Drive - Google Inc.
| Camp | Valor |
|------|------|
| Nom | Google Drive |
| Enllaç | https://drive.google.com/drive/ |
| Desenvolupador | Google Inc. |
| Funció | Emmagatzematge al núvol |
| Àrea | Productivitat (PROD) |
| Descripció | Emmagatzematge i col·laboració de fitxers al núvol. |

## Google Meet - Google Inc.
| Camp | Valor |
|------|------|
| Nom | Google Meet |
| Enllaç | https://meet.google.com/landing |
| Desenvolupador | Google Inc. |
| Funció | Videoconferència |
| Àrea | Comunicació (COM) |
| Descripció | Videoconferències amb qualitat alta i compartició de pantalla. |

## Google Search - Google Inc.
| Camp | Valor |
|------|------|
| Nom | Google Search |
| Enllaç | https://www.google.com/ |
| Desenvolupador | Google Inc. |
| Funció | Motor de cerca |
| Àrea | Referència (REF) |
| Descripció | Cercador web amb resultats rellevants i funcions avançades. |

## Google Translate - Google LLC
| Camp | Valor |
|------|------|
| Nom | Google Translate |
| Enllaç | https://translate.google.com/ |
| Desenvolupador | Google LLC |
| Funció | Traductor |
| Àrea | Llengua (LLGA) |
| Descripció | Traducció multilingüe amb IA en text, veu i imatges. |

## Jitsi - 8x8
| Camp | Valor |
|------|------|
| Nom | Jitsi |
| Enllaç | https://jitsi.org/ |
| Desenvolupador | 8x8 |
| Funció | Videoconferència |
| Àrea | Treball en equip (EQUIP) |
| Descripció | Videotrucades gratuïtes i de codi obert. |

## Lufi - Luc Didry
| Camp | Valor |
|------|------|
| Nom | Lufi |
| Enllaç | https://upload.disroot.org/ |
| Desenvolupador | Luc Didry |
| Funció | Compartició de fitxers |
| Àrea | Productivitat (PROD) |
| Descripció | Transferència de fitxers xifrats i temporals. |

## Meteocat - Servei Meteorològic de Catalunya
| Camp | Valor |
|------|------|
| Nom | Meteocat |
| Enllaç | https://m.meteo.cat/ |
| Desenvolupador | Servei Meteorològic de Catalunya |
| Funció | Meteorologia |
| Àrea | Meteorologia (METEO) |
| Descripció | Previsions meteorològiques i dades de Catalunya. |

## Moovitapp
| Camp | Valor |
|------|------|
| Nom | Moovitapp |
| Enllaç | https://moovit.com/ |
| Desenvolupador | Moovit App Global Ltd. |
| Funció | Transport públic |
| Àrea | Mobilitat i transport (MO-TR) |
| Descripció | Rutes de transport públic en ciutats. |

## Padlet
| Camp | Valor |
|------|------|
| Nom | Padlet |
| Enllaç | https://padlet.com/ |
| Desenvolupador | Padlet |
| Funció | Tauler virtual |
| Àrea | Treball en equip (EQUIP) |
| Descripció | Taulers col·laboratius visuals amb IA. |

## Settle Up
| Camp | Valor |
|------|------|
| Nom | Settle Up |
| Enllaç | https://settleup.io/ |
| Desenvolupador | Step Up Labs, Inc. |
| Funció | Despeses compartides |
| Àrea | Lleure i vida activa (LL-VA) |
| Descripció | Gestió de despeses en grup. |

## Smodin
| Camp | Valor |
|------|------|
| Nom | Smodin |
| Enllaç | https://smodin.io/ca |
| Desenvolupador | Smodin |
| Funció | Assistent d’escriptura |
| Àrea | Llengua (LLGA) |
| Descripció | IA per escriure, resumir i traduir textos. |

## TMB App - Transports Metropolitans de Barcelona
| Camp | Valor |
|------|------|
| Nom | TMB App |
| Enllaç | https://www.tmb.cat/ca/home |
| Desenvolupador | Transports Metropolitans de Barcelona |
| Funció | Transport públic |
| Àrea | Mobilitat i transport (MO-TR) |
| Descripció | Informació de metro, bus i bicicleta a Barcelona. |

## Viquipèdia - Fundació Wikimedia
| Camp | Valor |
|------|------|
| Nom | Viquipèdia |
| Enllaç | https://ca.wikipedia.org/wiki/Portada |
| Desenvolupador | Fundació Wikimedia |
| Funció | Enciclopèdia |
| Àrea | Referència (REF) |
| Descripció | Enciclopèdia col·laborativa de contingut lliure. |

## Viccionari - Fundació Wikimedia
| Camp | Valor |
|------|------|
| Nom | Viccionari |
| Enllaç | https://ca.wiktionary.org/wiki/Viccionari:Portada |
| Desenvolupador | Fundació Wikimedia |
| Funció | Diccionari |
| Àrea | Llengua (LLGA) |
| Descripció | Diccionari col·laboratiu amb definicions i traduccions. |

## Windy - Windyty
| Camp | Valor |
|------|------|
| Nom | Windy |
| Enllaç | https://www.windy.com/ |
| Desenvolupador | Windyty |
| Funció | Meteorologia |
| Àrea | Meteorologia (METEO) |
| Descripció | Visualització de dades meteorològiques en temps real. |

## Waze
| Camp | Valor |
|------|------|
| Nom | Waze |
| Enllaç | https://www.waze.com/ca/live-map |
| Desenvolupador | Waze Mobile Ltd |
| Funció | Navegació GPS |
| Àrea | Mobilitat i transport (MO-TR) |
| Descripció | Navegació amb dades de trànsit en temps real. |
